/**
 * GD remote Upload v1.1 Back-End
 * Remote-Upload files from links to Google Drive using Google Apps Script.
 * Currently bypassing the 50 MB URL Fetch response size by splitting the file.
 * https://github.com/AlirezaM1376/GDRemoteUpload
 * https://git.theazizi.ir/AlirezaM/GDRemoteUpload
 */
...
function processLink(data) {
  ...
  var maxSizeInBytes = 49.5 * 1024 * 1024; // URL Fetch response max size is 50 MB
  var uploadMethod = 0;
  ...
  try {
    ...
    // ---------- The Default Folder ----------
    if (folderId == "") {
      var folderIterator = DriveApp.getFoldersByName("GDRemoteUpload");
      var defaultFolder;
      if (folderIterator.hasNext()) {
        defaultFolder = folderIterator.next();
      } else {
        defaultFolder = DriveApp.createFolder("GDRemoteUpload");
      }
    ...
    if (!uploadMethod) {
      // If file size is under 50 MB save it directly
      var downloadOptions = {
        "method": "get",
        "muteHttpExceptions": true,
